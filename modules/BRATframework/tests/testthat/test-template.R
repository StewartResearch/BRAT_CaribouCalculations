...
#    `functionName` is a function's name in your module (e.g., `BRATframeworkEvent1`).
# 2. Copy this file to the tests folder (i.e., `C:/Users/admin-fstewart/Desktop/GitHub/BRAT_CaribouCalculations-master/modules/BRATframework/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("BRATframework")
  path <- list(modulePath = "C:/Users/admin-fstewart/Desktop/GitHub/BRAT_CaribouCalculations-master/modules",
               outputPath = file.path(tempdir(), "outputs"))
  parameters <- list(
    #.progress = list(type = "graphical", interval = 1),
    .globals = list(verbose = FALSE),
    BRATframework = list(N = "BLA")
  )
  ...
  testthat::expect_error(mySim <- simInit(times = times,
                   params = parameters,
                   modules = module,
                   objects = objects,
                   paths = path))
  ...
  if (exists("BRATframeworkEvent1", envir = .GlobalEnv)) {
    simOutput <- BRATframeworkEvent1(mySim)
  } else {
  ...
  if (exists("BRATframeworkEvent2", envir = .GlobalEnv)) {
    simOutput <- BRATframeworkEvent2(mySim)
  } else {
