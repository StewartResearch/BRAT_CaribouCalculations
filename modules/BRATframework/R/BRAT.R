...
env <- environment()
    ...
    message("This is the Threat1 top event: ", Threat1_topEvent)
  ...
  # the potential for mitigation to affect lambda. This goes into the MITIGATE() and is returned from here in the BRAT()
  mitigate_restoration <- 1-(Threat_LambdaEffect[[1]][2] + (0.5*Threat_LambdaEffect[[1]][1]))
  ...
  #Mitigation potential of the wolfCullEffect for the mitigate function. Return this at the end of the BRAT()
  mitigate_cull <- 1-wolfCullEffect
...
message("Are the risk threats to caribou prevented? ", TopEvent_lambda <- lambdaQuartile < 1 + (1-topEvent))
   }
Returnvalues<-list(topEvent = topEvent, mitigate_cull = mitigate_cull,  mitigate_restoration = mitigate_restoration)
return(Returnvalues)
}
