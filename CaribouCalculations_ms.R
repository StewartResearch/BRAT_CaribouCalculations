...
Threat3_multiplier = MortSadF*0.5 + 0.025 # we split the remaining 10% of the variation in SadF between the final two threats. We also split the remaining 5% of rec between the final two threats.
...
### Initial Frequency
# multiply it by what values exist in the barrier to back calculte this value
Threat4_InitialFreq <- Threat4_multiplier*(prod(0.95, 0.95, 0.95, 1.0, 1))


#######################################################
# step 6: Convert the inital frequency values to values of lambda:

Threat1_barriers <- c(1.5, 1.1, 0.65, 1.05, 1.0, 0.80, 1.0) # in units of Initial Frequency
Threat2_barriers <- c(2.19) # in units of Initial Frequency
Threat3_barriers <- c(0.95* 1* 1) # in units of Initial Frequency
Threat4_barriers <- c(0.95, 0.95, 0.95, 1.0, 1)  
...
#####
# repeat for other populations by altering the data in Step 1
####
...
###########################
###########################
# Maxhamish herd (STUDY AREA 2)  - NOT USED ---- 
###########################

###############################################################################
# Step 1: know (or estimate) some basic information about each study area/herd. 
### For Snake herd: averages from ECCC 2008 report as data is not available for Maxhamish/Calendar in this report
N = 600 # population size - just made up to be different than the other two study areas
...
(topEvent <- topEventCalculator(threatCalculator(Threat1_InitialFreq, Threat1_barriers), # climate change scenario
                                ...
                                threatCalculator(Threat4_InitialFreq, Threat4_barriers)))
message("This is the top event lambda: ", round(1 + (1 - topEvent), 3)) # this should be around 0.93 (ECCC 2008), but slightly lower as we know that
# Maxhamish/Calendar herd is declining (ie, below our threshold of 0.97 for 40% probability of stability)
###
...
###########################
###########################
# snake-sahtahneh herd (STUDY AREA 3) - DONE ---- 
###########################
...
Threat1_multiplier = MortSadF*0.90 # I'm assuming that 90% of the variation in SadF is due to predation. This can be changed.
Threat2_multiplier = Mortrecr*0.95 # <5% of random juvenile deaths are due to factors other than predation (calculated below based on maternity pen literature and pers coms)
Threat3_multiplier = MortSadF*0.5 + 0.025 # we split the remaining 20% of the variation in SadF between the final two threats. We also split the remaining 5% of rec between the final two threats.
Threat4_multiplier = Threat3_multiplier
...
Threat1_barriers <- c(2.0, 1.8, 0.65, 1.8, 1.0, 0.975, 1.0) # in units of Initial Frequency
Threat2_barriers <- c(2.19) # in units of Initial Frequency
